/**
 * Listed Import - Import listed HKEX IPO deals from Excel + scrape banks from prospectuses
 * Supports both Main Board and GEM Board
 *
 * Usage:
 *   npx tsx src/listed-import.ts              # Run full import (Main Board)
 *   npx tsx src/listed-import.ts --test 10   # Test on first 10 deals
 *   npx tsx src/listed-import.ts --resume    # Resume from last position
 *   BOARD=gem npx tsx src/listed-import.ts   # Import GEM Board deals
 */
...
// Board configuration
type Board = 'mainBoard' | 'gem';
const BOARD: Board = (process.env.BOARD as Board) || 'mainBoard';

// Configuration based on board
const EXCEL_PATH = BOARD === 'gem'
  ? path.join(__dirname, '../../Reference files/GEM Board/Listed/HKEX_IPO_Listed.xlsx')
  : path.join(__dirname, '../../Reference files/Main Board/Listed/HKEX_IPO_Listed.xlsx');
const PROGRESS_FILE = path.join(__dirname, `../.listed-import-progress-${BOARD}.json`);
const RESULTS_FILE = path.join(__dirname, `../.listed-import-results-${BOARD}.json`);
const URL_OVERRIDES_FILE = path.join(__dirname, '../url-overrides.json');
const TEMP_PDF_DIR = '/tmp/listed-pdfs';
...
async function saveDealToDatabase(
  ...
  try {
    ...
    // Upsert deal (status = 'listed' for listed deals)
    // Include board column based on BOARD env var
    const dealResult = await client.query(`
      INSERT INTO deals (company_id, status, listing_date, prospectus_url, board)
      VALUES ($1, 'listed', $2, $3, $4)
      ON CONFLICT ON CONSTRAINT deals_company_id_key DO UPDATE SET
        ...
        board = COALESCE(EXCLUDED.board, deals.board),
        updated_at = NOW()
      RETURNING id
    `, [companyId, listingDate, deal.prospectusUrl, BOARD]);
